...
from sim_with_mujoco.rl.models.dynamics_dvrk import DvrkRBFEKFDynamics
...
def main():
    ...
    centers[:, :, 1] = rng.uniform(
        -ACTION_LIMIT[:, None], ACTION_LIMIT[:, None], size=(7, num_basis)
    )
    centers[:, 0, 1] = 0.0
    widths = (
        base_width[:, None] * rng.uniform(0.75, 1.25, size=(7, num_basis))
    ).astype(np.float32)
    dynamics = DvrkRBFEKFDynamics(
        centers, widths, weights=np.ones((7, num_basis), dtype=np.float32)
    )
