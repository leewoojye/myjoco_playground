...
def main():
    ...
    planner = DvrkSCPSMPPIPlanner(
        ...
        num_samples=512,
        horizon=horizon,
        # SMPPI is applied only to these sparse lifted control points; the
        # remaining rollout steps are generated by cubic spline interpolation.
    ...
    try:
        for step in range(env.max_steps):
            ...
            step_trace = {
                ...
                "rollout_derivative": planner.candidate_derivatives.detach().cpu().clone(),
                "rollout_control_point_noise": (planner.candidate_control_point_noise.detach().cpu().clone()),
                "rollout_importance_cost": planner.candidate_importance_cost.detach().cpu().clone(),
                "rollout_t_axis_cost": planner.candidate_t_axis_cost.detach().cpu().clone(),
                "rollout_action_limit_cost": (planner.candidate_action_limit_cost.detach().cpu().clone()),
                ...
                "action_control_points_after": (planner.nominal_action_control_points.detach().cpu().clone()),
                "derivative_control_points_after": (planner.nominal_derivative_control_points.detach().cpu().clone()),
                "spline_support_indices": planner.support_indices.detach().cpu().clone(),
            }
